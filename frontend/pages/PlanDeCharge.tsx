...
export default function PlanDeCharge() {
  ...
  // Données mockées et déterministes pour l'exemple (pas de backend ici)
  const gryzzlyFor = useMemo(() => {
    const map: Record<string, { date: string; projet: string; heures: number }[]> = {};
    actifs.forEach((c) => {
      const arr: { date: string; projet: string; heures: number }[] = [];
      days.forEach((day) => {
        if (!day.isWeekend && !day.isHoliday) {
          if (day.d % 5 === 0) arr.push({ date: dateKey(day.d), projet: "Projet A", heures: 7 });
          else if (day.d % 7 === 0) arr.push({ date: dateKey(day.d), projet: "Projet B", heures: 3.5 });
        }
      });
      map[c.id] = arr;
    });
    return map;
  }, [actifs, days, monthKey]);
  ...
  const absencesFor = useMemo(() => {
    ...
    actifs.forEach((c) => {
      ...
      days.forEach((day) => {
        if (!day.isWeekend && !day.isHoliday) {
          if (day.d % 11 === 0) arr.push({ date: dateKey(day.d), type: "RTT", jours: 1 });
          else if (day.d % 13 === 0) arr.push({ date: dateKey(day.d), type: "Congés", jours: 1 });
        }
      });
      map[c.id] = arr;
    });
                ...
                return (
                  <>
                        ...
                        return (
                          <td key={`${c.id}-${day.d}`} className={`h-8 text-center ${day.isWeekend || day.isHoliday ? 'bg-muted' : ''}`}>
                            {!day.isWeekend && !day.isHoliday ? (
                              <span className="mx-auto inline-flex items-center justify-center rounded-sm bg-muted px-1.5 py-0.5 text-xs tabular-nums text-muted-foreground">
                                7
                              </span>
                            ) : null}
                          </td>
                    ...
                    {expanded[c.id] && (() => {
                      ...
                      (absencesFor[c.id] || []).forEach((a) => {
                        absMap[a.date] = (absMap[a.date] || 0) + a.jours * 7;
                      });
                      if (Object.keys(absMap).length) {
                        lines.push({ label: 'Absences (Payfit)', kind: 'absence', hoursByDay: absMap });
                      }

                      // Imputations groupées par projet
                      const imputationsByProject: Record<string, Record<string, number>> = {};
                      (gryzzlyFor[c.id] || []).forEach((g) => {
                        if (!imputationsByProject[g.projet]) imputationsByProject[g.projet] = {};
                        imputationsByProject[g.projet][g.date] = (imputationsByProject[g.projet][g.date] || 0) + g.heures;
                      });
