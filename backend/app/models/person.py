...
class Person(BaseModel, SoftDeleteMixin):
    ...
    identifiers = relationship(
        "PersonIdentifier", back_populates="person", cascade="all, delete-orphan"
    )
    engagements = relationship(
        "Engagement", back_populates="person", cascade="all, delete-orphan"
    )
    team_memberships = relationship(
        "TeamMember", back_populates="person", cascade="all, delete-orphan"
    )
    project_memberships = relationship(
        "ProjectMember", back_populates="person", cascade="all, delete-orphan"
    )
    task_assignments = relationship(
        "TaskAssignee", back_populates="person", cascade="all, delete-orphan"
    )
    allocations = relationship(
        "Allocation", back_populates="person", cascade="all, delete-orphan"
    )
    capacities = relationship(
        "Capacity", back_populates="person", cascade="all, delete-orphan"
    )
    absences = relationship(
        "Absence", back_populates="person", cascade="all, delete-orphan"
    )
    benefits = relationship(
        "PersonBenefit", back_populates="person", cascade="all, delete-orphan"
    )
...
class PersonIdentifier(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "org_id", "id_type", "id_value", name="uq_person_identifiers_org_type_value"
        ),
    ...
    )  # payfit_employee_id, payroll_number, etc.
...
class Engagement(BaseModel):
    ...
    type = Column(
        Enum(
            "employee", "contractor", "mandataire", "freelance", name="engagement_type"
        ),
        nullable=False,
        default="employee",
    )
...
class User(BaseModel, SoftDeleteMixin):
    ...
    roles = relationship(
        "UserOrgRole", back_populates="user", cascade="all, delete-orphan"
    )
    refresh_tokens = relationship(
        "RefreshToken", back_populates="user", cascade="all, delete-orphan"
    )
    api_tokens = relationship(
        "ApiToken", back_populates="created_by_user", cascade="all, delete-orphan"
    )
    audit_logs = relationship("AuditLog", back_populates="actor")
    comments = relationship("Comment", back_populates="author")
    payfit_employee = relationship(
        "PayfitEmployee", back_populates="local_user", uselist=False
    )
