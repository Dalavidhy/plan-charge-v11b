...
class ExternalProvider(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "org_id", "provider_key", name="uq_external_providers_org_key"
        ),
    )
    ...
    connections = relationship(
        "ExternalConnection", back_populates="provider", cascade="all, delete-orphan"
    )
    external_accounts = relationship(
        "ExternalAccount", back_populates="provider", cascade="all, delete-orphan"
    )
    sync_jobs = relationship(
        "SyncJob", back_populates="provider", cascade="all, delete-orphan"
    )
    sync_events = relationship(
        "SyncEvent", back_populates="provider", cascade="all, delete-orphan"
    )
...
class ExternalConnection(BaseModel):
    ...
    provider_id = Column(
        UUID(as_uuid=True),
        ForeignKey("external_providers.id"),
        nullable=False,
        index=True,
    )
    ...
    meta_data = Column(
        JSON, nullable=True
    )  # Renamed from metadata (reserved in SQLAlchemy 2.0)
...
class ExternalAccount(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider_id", "external_user_id", name="uq_external_accounts_provider_user"
        ),
    ...
    identity_links = relationship(
        "IdentityLink", back_populates="external_account", cascade="all, delete-orphan"
    )
...
class IdentityLink(BaseModel):
    ...
    external_account_id = Column(
        UUID(as_uuid=True), ForeignKey("external_accounts.id"), nullable=False
    )
...
class StgRoster(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider_id", "external_user_id", name="uq_stg_roster_provider_user"
        ),
    )
...
class StgAbsence(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider_id",
            "external_absence_id",
            name="uq_stg_absences_provider_absence",
        ),
...
class StgTimesheet(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider_id", "external_entry_id", name="uq_stg_timesheets_provider_entry"
        ),
