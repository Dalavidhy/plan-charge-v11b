...
class TREligibilityOverride(BaseModel):
    ...
    collaborator_id = Column(String(255), nullable=True, index=True)  # Gryzzly ID or Payfit prefixed ID
    email = Column(String(255), nullable=False, index=True, unique=True)  # Primary key for override
    ...
    __table_args__ = (
        UniqueConstraint('email', name='uq_tr_eligibility_email'),
    )