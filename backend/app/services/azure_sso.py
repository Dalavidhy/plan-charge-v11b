...
class AzureSSO:
    ...
    async def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in result:
                ...
                logger.error(f"Azure AD token exchange error - Code: {error_code}, Description: {error_msg}")
                logger.error(f"Full error response: {result}")
                
                # Provide more specific error messages
                if "AADSTS700" in error_msg:
                    detail = "Configuration error: Check your redirect URI in Azure Portal"
                    ...
                    detail = "Invalid client credentials. Check your Client ID and Secret."
                else:
                    detail = error_msg
    ...
    async def get_or_create_user(
        ...
        email = azure_user_info.get("preferred_username") or azure_user_info.get("email")
