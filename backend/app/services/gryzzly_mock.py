...
class GryzzlyMockService:
    ...
    def _generate_collaborators(self, count: int = 15) -> List[Dict]:
        ...
        collaborators = []
        departments = ["Engineering", "Design", "Product", "Marketing", "Sales", "Finance"]
        positions = ["Developer", "Designer", "Manager", "Analyst", "Consultant", "Director"]
        
        first_names = ["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Henry", 
                       "Iris", "Jack", "Kate", "Leo", "Maya", "Noah", "Olivia"]
    ...
    def _generate_projects(self, count: int = 20) -> List[Dict]:
        ...
        clients = ["Acme Corp", "TechStart", "Global Industries", "Innovate Ltd", "Digital Solutions",
                   "Future Systems", "Smart Tech", "Cloud Nine", "Data Dynamics", "AI Ventures"]
        
        project_names = ["Website Redesign", "Mobile App Development", "API Integration", 
                        "Data Migration", "Security Audit", "Performance Optimization",
                        "Cloud Migration", "DevOps Setup", "UI/UX Refresh", "Backend Refactoring",
                        "Analytics Dashboard", "E-commerce Platform", "CRM Implementation",
                        "Machine Learning POC", "Blockchain Integration", "IoT Platform",
                        "Microservices Architecture", "Real-time System", "Payment Gateway",
        ...
        for i in range(count):
            ...
            projects.append({
                ...
                "clientName": random.choice(clients) if random.random() > 0.3 else None,
                "projectType": random.choice(project_types),
                "startDate": start_date.date().isoformat(),
                "endDate": end_date.date().isoformat() if random.random() > 0.3 else None,
                "isActive": random.random() > 0.2,  # 80% active
                "isBillable": random.random() > 0.3,  # 70% billable
                "budgetHours": random.randint(40, 1000) if random.random() > 0.4 else None,
                "budgetAmount": random.randint(5000, 100000) if random.random() > 0.5 else None,
                "createdAt": (datetime.now() - timedelta(days=random.randint(30, 365))).isoformat(),
                "updatedAt": datetime.now().isoformat()
            })
    ...
    def _generate_tasks(self, count: int = 50) -> List[Dict]:
        """Generate mock tasks"""
        tasks = []
        task_types = ["Development", "Design", "Testing", "Documentation", "Meeting", "Review", "Research"]
        
        task_names = ["Frontend Development", "Backend Development", "Database Design", 
                      "API Development", "Unit Testing", "Integration Testing", "Code Review",
                      "Documentation", "Sprint Planning", "Daily Standup", "Retrospective",
                      "Bug Fixing", "Performance Tuning", "Security Review", "Deployment",
                      "User Research", "Wireframing", "Prototyping", "UI Design", "UX Testing"]
        ...
        for i in range(count):
            ...
            tasks.append({
                ...
                "estimatedHours": random.randint(4, 80) if random.random() > 0.3 else None,
    ...
    def _generate_declarations(self, count: int = 200) -> List[Dict]:
        ...
        descriptions = ["Development work", "Bug fixing", "Code review", "Testing", 
                       "Documentation", "Meeting", "Research", "Design work", "Planning",
                       "Deployment", "Support", "Training", "Analysis", "Optimization"]
        
        for i in range(count):
            collaborator = random.choice(self.collaborators)
            project = random.choice(self.projects)
            task = random.choice([t for t in self.tasks if t["projectId"] == project["id"]])
            
            declaration_date = datetime.now().date() - timedelta(days=random.randint(0, 60))
            ...
            declarations.append({
                ...
                "comment": f"Work on {task['name']}" if random.random() > 0.5 else None,
                "status": status,
                "approvedBy": random.choice(self.collaborators)["email"] if status == "approved" else None,
                "approvedAt": datetime.now().isoformat() if status == "approved" else None,
                "isBillable": project["isBillable"],
                "billingRate": random.randint(50, 200) if project["isBillable"] else None,
    ...
    async def create_declaration(
        ...
        new_declaration = {
            ...
            "status": "draft",
            "isBillable": next((p["isBillable"] for p in self.projects if p["id"] == project_id), False),
    ...
    async def trigger_sync(self, sync_type: str = "full") -> Dict:
        ...
        return {
            ...
            "recordsSynced": {
                "collaborators": len(self.collaborators) if sync_type in ["full", "collaborators"] else 0,
                "projects": len(self.projects) if sync_type in ["full", "projects"] else 0,
                "tasks": len(self.tasks) if sync_type in ["full", "tasks"] else 0,
                "declarations": len(self.declarations) if sync_type in ["full", "declarations"] else 0
            }
        }
    ...
    async def get_time_report(
        ...
        """Get time report"""
        declarations = await self.get_declarations(start_date=start_date, end_date=end_date)
    ...
    async def get_billing_report(
        ...
        total_hours = sum(d["durationHours"] + d["durationMinutes"] / 60 for d in billable_declarations)
        total_amount = sum(
            (d["durationHours"] + d["durationMinutes"] / 60) * (d.get("billingRate", 100))
    ...
    async def search_collaborators(self, query: str) -> List[Dict]:
        ...
        return [
            ...
            if query_lower in c["email"].lower() or
               query_lower in c["firstName"].lower() or
               query_lower in c["lastName"].lower()
        ]
    ...
    async def get_activity_summary(
        ...
        total_hours = sum(d["durationHours"] + d["durationMinutes"] / 60 for d in declarations)
