...
class GryzzlyAPIClient:
    ...
    async def _make_request(
        ...
        try:
            ...
            if retry_count < self.max_retries:
                ...
                logger.error(f"Request timeout after {self.max_retries} retries: {str(e)}")
                raise
    ...
    async def get_declarations(
        ...
        if USE_MOCK:
            return await mock_service.get_declarations(start_date, end_date, collaborator_id, project_id, status)
        ...
        for user in users:
            ...
            try:
                ...
                for decl in declarations:
                    ...
                    project_id_from_task = task_project_map.get(task_id) if task_id else None
                    is_billable_from_task = task_billable_map.get(task_id, False) if task_id else False
                    ...
                    all_declarations.append({
                        ...
                        "durationHours": decl.get("duration", 0) / 3600 if decl.get("duration") else 0,
    ...
    async def create_declaration(
        ...
        if USE_MOCK:
            return await mock_service.create_declaration(collaborator_id, project_id, task_id, date, hours, description)
    ...
    async def update_declaration(
        ...
        if USE_MOCK:
            return await mock_service.update_declaration(declaration_id, hours, description, status)
    ...
    async def search_collaborators(self, query: str) -> List[Dict]:
        ...
        return [
            u for u in users
            if query_lower in u.get("email", "").lower() or
               query_lower in u.get("firstName", "").lower() or
               query_lower in u.get("lastName", "").lower()
        ]
    ...
    async def search_projects(self, query: str) -> List[Dict]:
        ...
        return [
            ...
            if query_lower in p.get("name", "").lower() or
               query_lower in (p.get("code") or "").lower()
        ]
    ...
    async def get_billing_report(
        ...
        """Get billing report"""
        if USE_MOCK:
            return await mock_service.get_billing_report(start_date, end_date, project_id)
    ...
    async def get_activity_summary(
        ...
        if USE_MOCK:
            return await mock_service.get_activity_summary(collaborator_id, start_date, end_date)
        ...
        return {
            ...
            "averageHoursPerDay": total_hours / ((end_date - start_date).days + 1) if (end_date - start_date).days > 0 else total_hours
        }