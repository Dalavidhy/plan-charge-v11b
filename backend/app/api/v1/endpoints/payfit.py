...
async def get_payfit_stats(
    ...
    active_contracts_result = await session.execute(
        select(func.count())
        .select_from(PayfitContract)
        .where(PayfitContract.is_active == True)
    )
    active_contracts = active_contracts_result.scalar() or 0

    # Count absences
    total_absences_result = await session.execute(
        select(func.count()).select_from(PayfitAbsence)
    )
    ...
    pending_absences_result = await session.execute(
        select(func.count())
        .select_from(PayfitAbsence)
        .where(PayfitAbsence.status == "pending")
    )
...
async def test_payfit_connection(
    ...
    try:
        ...
        if not connected:
            raise HTTPException(
                status_code=503, detail="Unable to connect to Payfit API"
            )
...
async def sync_employees(
    ...
    if running_sync:
        raise HTTPException(status_code=400, detail="Employee sync is already running")
...
async def sync_contracts(
    ...
    if running_sync:
        raise HTTPException(status_code=400, detail="Contract sync is already running")
...
async def sync_absences(
    ...
    if running_sync:
        raise HTTPException(status_code=400, detail="Absence sync is already running")
    ...
    try:
        ...
        # Start sync in background
        background_tasks.add_task(sync_service.sync_absences, start_date, end_date)
...
async def sync_full(
    ...
    result = await session.execute(
        select(func.count())
        .select_from(PayfitSyncLog)
        .where(PayfitSyncLog.sync_status == "started")
    )
    running_count = result.scalar()

    if running_count > 0:
        raise HTTPException(
            status_code=400, detail="A synchronization is already running"
        )
    ...
    return [
        {
            ...
            "last_synced_at": (
                emp.last_synced_at.isoformat() if emp.last_synced_at else None
            ),
    ...
    return [
        {
            ...
            "employee_first_name": (
                contract.employee.first_name if contract.employee else None
            ),
            "employee_last_name": (
                contract.employee.last_name if contract.employee else None
            ),
            "employee_email": contract.employee.email if contract.employee else None,
            "employee_full_name": (
                f"{contract.employee.first_name} {contract.employee.last_name}"
                if contract.employee
                and contract.employee.first_name
                and contract.employee.last_name
                else None
            ),
            ...
            "last_synced_at": (
                contract.last_synced_at.isoformat() if contract.last_synced_at else None
            ),
            "created_at": (
    ...
    return [
        {
            ...
            # Employee information
            "employee_first_name": (
                absence.employee.first_name if absence.employee else None
            ),
            "employee_last_name": (
                absence.employee.last_name if absence.employee else None
            ),
            ...
            "employee_full_name": (
                ...
                if absence.employee
                and absence.employee.first_name
                and absence.employee.last_name
                else None
            ),
            # Absence information
            ...
            "last_synced_at": (
                absence.last_synced_at.isoformat() if absence.last_synced_at else None
            ),
            "created_at": (
