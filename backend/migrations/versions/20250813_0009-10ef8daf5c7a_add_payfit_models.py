...
Revision ID: 10ef8daf5c7a
Revises: 74b7c8174dd8
Create Date: 2025-08-13 00:09:16.296767
...
revision = "10ef8daf5c7a"
down_revision = "74b7c8174dd8"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_payfit_employees_email"), "payfit_employees", ["email"], unique=False
    )
    op.create_index(
        op.f("ix_payfit_employees_payfit_id"),
        "payfit_employees",
        ["payfit_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["payfit_employee_id"],
            ["payfit_employees.payfit_id"],
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("payfit_id"),
    )
    op.create_index(
        op.f("ix_payfit_contracts_payfit_id"),
        "payfit_contracts",
        ["payfit_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_payfit_absences_payfit_id"),
        "payfit_absences",
        ["payfit_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "sync_metadata", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("created_at", sa.DateTime(), nullable=True),
        sa.PrimaryKeyConstraint("id"),
    )
